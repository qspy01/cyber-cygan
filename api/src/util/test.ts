...
const getTestPath = (service: string) => path.join('./src/util/tests/', `./${service}.json`);
const getTests = (service: string): Test[] | false => loadJSON(getTestPath(service));
...
const runTestsFor = async (service: string) => {
    ...
    for (const test of tests) {
        ...
        try {
            ...
            const errorString = (e instanceof Error ? e.message : String(e)).split('\n');
            let c = '┃';
            errorString.forEach((line: string, index: number) => {
...
const printHeader = (service: string, padLen: number) => {
...
switch (action) {
    ...
    default:
        ...
        console.log(
            Bright('total fails:'),
            Object.values(failCounters).reduce((a: number, b: number) => a + b, 0)
        );
